...
- **Let the builder load it.** The `builder` agent is granted `activate_skill`. Name the
  skill on the prompt's `Load skill:` line and let the builder load it into its own
  disposable context — the cheaper option, since the framework guidance never enters the
  orchestrator's context at all.
