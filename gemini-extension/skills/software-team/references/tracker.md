...
Tickets live in `.gemini/state/tracker/`. That path sits inside the office-state
carve-out `SKILL.md` grants the orchestrator — creating and updating a ticket file is the
same office-state write as a decision-log line, so no builder spawn. Project files are
the opposite case: `CONTEXT.md`, `docs/adr/`, and anything else a human would call project
documentation go through a builder spawn like any other file
(`references/unsettled-requirements.md`).

## Ticket shape

One ticket, one file: `.gemini/state/tracker/<id>-<slug>.md`. `<id>` is the stable
...
No index file — `ls .gemini/state/tracker/` **is** the index. Exactly two things in that
...
```
- YYYY-MM-DD: <decision or shipped behavior> — <why> (.gemini/state/tracker/<id>-<slug>.md)
```
