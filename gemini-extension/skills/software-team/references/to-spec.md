...
Write to `.gemini/state/tracker/<slug>-spec.md`:
...
Decisions:
- <settled architectural/implementation choice> — <why> (.gemini/state/tracker/<id>-<slug>.md)
...
The spec file, `MAP.md`, and ticket files are all under `.gemini/state/tracker/`, inside the
office-state carve-out `SKILL.md` grants — the orchestrator writes them directly, same as
