...
| `builder` (`Mode: TDD` for bug fixes / TDD plans; docs the criteria call for are its work) | ✓ cheapest tier | ✓ mid tier | ✓ | ✓ top tier |
...
- Read-only passes over the same finished diff (security-reviewer, second review,
...
2. Spawn one `builder` role at the cheapest tier — role contract from
   `references/roles.md` plus a four-field prompt: `Task:` / `Files:` / one
   `Acceptance criterion:` / `Out of scope:`, plus `Report back in:` the human's
   language.
...
Prior gates: <verifier PASS / second review APPROVED / security-reviewer CLEAR — each
  that applies>
Report back in: <the human's language>
```
...
2. Every verdict the spawn matrix required came back clean — verifier `PASS`
   (`APPROVED` in `Mode: REVIEW`), second review `APPROVED`, security-reviewer `CLEAR`
