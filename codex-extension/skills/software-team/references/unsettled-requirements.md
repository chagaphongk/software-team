...
2. **Surface every genuinely open question as an explicit fork**, per the fork shape in
   ...
   the cost of each. Never a bare "what do you want?"
...
real project files, **not** office state: unlike `.software-team/state/tracker/` files
(`references/tracker.md`), the orchestrator does not write them directly. They go into the
PLAN's Steps as ratifications and are written by a `builder`-role spawn like any other
project file — the builder contract in `references/roles.md` counts them as documentation
the criteria called for.
