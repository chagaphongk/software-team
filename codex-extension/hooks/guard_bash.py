#!/usr/bin/env python3
"""PreToolUse guard for shell commands: blocks destructive/irreversible commands (exit 2).

Ported from the Claude Code version. The exact JSON field Codex puts the shell command
under, and whether PreToolUse fires before the command runs at all, were not confirmed by
live testing -- see hooks/PORT_NOTES.md. This checks every plausible field name/nesting
rather than assuming one, so it degrades gracefully instead of silently never firing if
the real key differs from all of these.
"""
...
# unattended shell spawn can do before that gate.
TEXT_BLOCK = [
