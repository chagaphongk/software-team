...
| `builder` (`Mode: TDD` for bug fixes / TDD plans; docs the criteria call for are its work) | ✓ haiku | ✓ sonnet | ✓ | ✓ opus |
...
- Read-only passes over the same finished diff (security-reviewer, Fable review,
...
2. Spawn one `software-team:builder` at haiku with a four-field prompt:
...
Prior gates: <verifier PASS / Fable APPROVED / security-reviewer CLEAR —
  each that applies>
Report back in: <the human's language>
```
...
2. Every verdict the spawn matrix required came back clean — verifier `PASS`
   (`APPROVED` in `Mode: REVIEW`), Fable `APPROVED`, security-reviewer `CLEAR` — with
   the evidence recorded (exact commands and results).
