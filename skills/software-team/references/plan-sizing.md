...
**Escalate to `references/wayfinder.md`** for work that won't finish in one session — a fix
or feature spanning sessions is exactly its shape, and specifically where a Phase Map
...
the fog into its map so charting doesn't start cold; its decision-ticket map then owns
the multi-session continuity instead of the Phase Map below. **The map replaces the Phase
Map, not the office loop**: each ticket the map marks ready to build comes back through
the office as a normal task — its own `## PLAN output shape`, tier, and BUILD → REVIEW →
...
run of two to four phases — the ordinary oversized case, lighter than a map —
