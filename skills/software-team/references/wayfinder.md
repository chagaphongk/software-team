...
Draft the map **before any ticket exists**, at `.claude/state/tracker/MAP.md` — the one
...
Decisions so far:
- <decision> (.claude/state/tracker/<id>-<slug>.md)
...
| `research` | AFK | Spawn `software-team:researcher` against the ticket — facts external to the working directory |
