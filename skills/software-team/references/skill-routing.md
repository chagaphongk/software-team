...
Selection is owned by the agent doing the work. Every skill-enabled role sees the full
available-skills listing (project, user-global, plugin-provided) in its own context at
startup — it does not need you to enumerate options. Your job is the `Skill hints:`
line: pass the exact handles you already know are relevant, or `none known`. Hints are
advisory, never commands — the agent takes a hint when it matches and replaces it when
its own listing shows a better fit. Never load a skill into your own context just to
choose for another agent; that spends orchestrator context on guidance only the
subagent needs.
...
This holds for repo-vendored skills too: `.claude/skills/` outranks a generic skill on
...
2. **The official/vendor skill for that exact technology** — the one published by the
   ...
   a handle that merely sounds vendor-like proves nothing.
