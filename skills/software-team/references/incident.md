...
1. **Triage** — `software-team:researcher`, read-only diagnosis: what broke, since
   when, blast radius, and which reversible mitigation is available. A diagnosis, not a
   fix.
2. **Mitigate** — human approval first (same bar as any deploy), then
   `software-team:deployer` runs a **reversible mitigation only**: rollback, restart,
   feature-disable. Never a new forward code/config fix — that becomes a normal T2
